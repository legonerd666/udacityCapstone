...
void Creator::Race()
{
    ...
    getline(cin, name, '\n');
    if (name == "")
        _threads.emplace_back(thread(&Character::SetRace, _character, "God"));
    else
        _threads.emplace_back(thread(&Character::SetRace, _character, move(name)));
#pragma region Ability Scores

    short abilityScores[6];
    abilityScores[0] = _character->GetAbilityScore(strength);
    abilityScores[1] = _character->GetAbilityScore(dexterity);
    abilityScores[2] = _character->GetAbilityScore(constitution);
    abilityScores[3] = _character->GetAbilityScore(intelligence);
    abilityScores[4] = _character->GetAbilityScore(wisdom);
    abilityScores[5] = _character->GetAbilityScore(charisma);
    {
...
#pragma region Speed
    ...
    _threads.emplace_back(thread(&Character::SetSpeed, _character, move(GetSpeed())));
...
void Creator::Role()
{
    ...
    // Adds Hitpoints
    _threads.emplace_back(thread(&Character::SetHitPoints, _character, move(GetHitDie())));
    ...
    // Sets Saves
    _threads.emplace_back(thread(&Character::SetSave, _character, fortitude, GetSave(fortitude)));
    _threads.emplace_back(thread(&Character::SetSave, _character, reflex, GetSave(reflex)));
    _threads.emplace_back(thread(&Character::SetSave, _character, will, GetSave(will)));
...
void Creator::SetGold()
{
    ...
    {
        stoi(goldPieces);
        _character->SetCurrency(gold, stoi(goldPieces));
    }
    catch (const std::invalid_argument &e)
    {
        if (goldPieces == "")
            _character->SetCurrency(gold, 0);
        else
        {
...
string Creator::FormattedCurrencies()
{
    ...
    for (short i = 0; i < 4; i++)
    {
        currencies += to_string(_character->GetCurrencyAmount(_character->CurrencyType(i)));
        currencies += " ";
        currencies += EnumToString(_character->CurrencyType(i));
        currencies += " piece";
        if (_character->GetCurrencyAmount(_character->CurrencyType(i)) != 1)
            currencies += "s";
        if (i < 3)
            currencies += ", ";
        else
...
{
    if (_character->GetCurrencyAmount(currencyType) < cost)
    {
        int CPs = _character->GetCurrencyAmount(copper);
        int SPs = _character->GetCurrencyAmount(silver);
        int GPs = _character->GetCurrencyAmount(gold);
        int PPs = _character->GetCurrencyAmount(platinum);
        ...
        case copper:
            ...
            _character->SetCurrency(copper, CPs - cost);
            _character->SetCurrency(silver, SPs);
            _character->SetCurrency(gold, GPs);
            _character->SetCurrency(platinum, PPs);
            return 0;
        case silver:
            while (SPs < cost)
            {
            ...
            _character->SetCurrency(copper, CPs);
            _character->SetCurrency(silver, SPs - cost);
            _character->SetCurrency(gold, GPs);
            _character->SetCurrency(platinum, PPs);
            return 0;
        case gold:
            ...
            _character->SetCurrency(copper, CPs);
            _character->SetCurrency(silver, SPs);
            _character->SetCurrency(gold, GPs - cost);
            _character->SetCurrency(platinum, PPs);
            return 0;
        case platinum:
            while (PPs < cost)
            ...
            _character->SetCurrency(silver, SPs);
            _character->SetCurrency(gold, GPs);
            _character->SetCurrency(platinum, PPs - cost);
            return 0;
        default:
            return -1;
        }
    ...
    {
        _character->SetCurrency(currencyType, _character->GetCurrencyAmount(currencyType) - cost);
        return 0;
    }
}
